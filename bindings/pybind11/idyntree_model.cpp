...
void modelClassDefinition(py::class_<Model>& model)
{
    model.def(py::init())
        ...
        .def("get_joint",
             ...
             py::return_value_policy::reference)
        .def("add_joint", py::overload_cast<const std::string&, IJointConstPtr>(&Model::addJoint))
        ...
        .def("compute_full_tree_traversal",
             [](Model& m, const LinkIndex) -> std::unique_ptr<Traversal> {
...
void traversalClassDefinition(py::class_<Traversal>& traversal)
{
    traversal.def(py::init())
        ...
        .def("get_base_link", &Traversal::getBaseLink, py::return_value_policy::reference)
        .def("get_parent_link", &Traversal::getParentLink, py::return_value_policy::reference)
        .def("get_parent_joint", &Traversal::getParentJoint, py::return_value_policy::reference)
...
void utilityFunctionsDefinition(pybind11::module& module)
{
    ...
    module
        .def("get_random_model",
             ...
             py::arg("nr_of_joints"),
             py::arg("nr_of_additional_frames") = 10,
...
void iDynTreeModelBindings(pybind11::module& module)
{
    ...
    py::class_<SolidShape>(module, "_SolidShape")
        ...
        .def_property("link_H_geometry",
                      &SolidShape::getLink_H_geometry,
                      &SolidShape::setLink_H_geometry)
        .def_property("material", &SolidShape::getMaterial, &SolidShape::setMaterial)
        .def("clone", &SolidShape::clone)
        .def(
            "as_external_mesh",
            [](SolidShape& s) -> ExternalMesh* {
        ...
        .def(
            ...
            [](SolidShape& s) -> Sphere* {
                ...
                if (mesh)
                    return mesh;
                raisePythonException(PyExc_ValueError, "Specified object is not a Sphere object.");
                return nullptr;
            },
            py::return_value_policy::reference);
    ...
    py::class_<ExternalMesh, SolidShape>(module, "ExternalMesh")
        .def(py::init())
        .def_property("filename", &ExternalMesh::getFilename, &ExternalMesh::setFilename)
    ...
    py::class_<LinkShapesWrapper>(module, "LinkShapes")
        .def("__len__", [](const LinkShapesWrapper& v) { return v.shapes->size(); })
        .def(
            "__getitem__",
        ...
        .def("append", [](LinkShapesWrapper& v, SolidShape* s) { v.shapes->push_back(s->clone()); })
        .def(
            "__iter__",
            [](LinkShapesWrapper& v) {
    ...
    py::class_<ModelShapesWrapper>(module, "ModelShapes")
        .def("__len__", [](const ModelShapesWrapper& v) { return v.shapes->size(); })
        .def("__getitem__",
             [](const ModelShapesWrapper& v, std::size_t index) -> LinkShapesWrapper {
    ...
    py::class_<ModelSolidShapes>(module, "ModelSolidShapes")
        ...
        .def("__len__", [](const ModelSolidShapes& v) { return v.getLinkSolidShapes().size(); })
        ...
        .def(
            ...
            [](ModelSolidShapes& shapes,
               std::pair<std::size_t, std::size_t> indices) -> SolidShape* {
                return shapes.getLinkSolidShapes().at(indices.first).at(indices.second);
            },
            py::return_value_policy::reference)
        .def("__setitem__",
             [](ModelSolidShapes& shapes,
                ...
                SolidShape* s) {
                 SolidShape* old_shape
                     = shapes.getLinkSolidShapes().at(indices.first).at(indices.second);
                 shapes.getLinkSolidShapes()[indices.first][indices.second] = s->clone();
                 delete old_shape;
             })
        .def_property_readonly("solid_shapes", [](ModelSolidShapes& shapes) -> ModelShapesWrapper {
