...
JointElement::JointElement(XMLParserState& parserState,
...
bool JointElement::setAttributes(
...
void JointElement::exitElementScope()
{
    ...
    if (info.joint && map)
    {
        ...
        if (!map->insert(
                    std::unordered_map<std::string, JointElement::JointInfo>::value_type(m_jointName,
                                                                                         info))
                 .second)
        {
