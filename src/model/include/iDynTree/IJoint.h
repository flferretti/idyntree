...
public:
    ...
    /**
     * Get the getNrOfPosCoords() × getNrOfDOFs() matrix that maps the joint velocity to the
     * derivative of the joint position.
     *
     ...
     */
    virtual bool
    getPositionDerivativeVelocityJacobian(const iDynTree::Span<const double> jntPos,
     ...
     */
    virtual Transform getRestTransform(const LinkIndex child, const LinkIndex parent) const = 0;
    ...
    getTransform(const VectorDynSize& jntPos, const LinkIndex child, const LinkIndex parent) const
        = 0;
    ...
    getMotionSubspaceVector(int dof_i, const LinkIndex child, const LinkIndex parent) const
        = 0;
    ...
    /**
     * Compute the internal torque of joint, given the internal wrench that the
     * linkThatAppliesWrench applies on the linkOnWhichWrenchIsApplied, expressed in the link frame
     * of the linkOnWhichWrenchIsApplied.
     *
     ...
     * @param[in] linkThatAppliesWrench link index of the link that applies the considered internal
     * wrench.
     * @param[in] linkOnWhichWrenchIsApplied link index of the link on which the considered internal
     * wrench is applied.
     ...
     *  case it may be even just instantaneous models (for example, when only the damping is
     * considered).
     *
     *  For the type of joint dynamics supported, see the iDynTree::JointDynamicsType enum
     * documentation.
     *
     *  The joint dynamics model are used in the following contexts:
     *   * In methods to serialize and deserialize URDF files
     *
     *  The joint dynamics are **not used at all** in classes to compute kinematics and dynamics
     * quantities, such as iDynTree::KinDynComputations .
     */
    ///@{
     ...
     * The damping coefficient is expressed in N∙s/m for a prismatic joint, N∙m∙s/rad for a revolute
     * joint.
     *
     ...
     * The static friction coefficient is expressed in N for a prismatic joint, N∙m for a revolute
     * joint.
     *
     * This parameter is considered in the following joint dynamics types:
     * * `URDFJointDynamics`
     ...
     * @param[inout] jntPos full vector of model joint positions, of dimension
     * Model::getNrOfPosCoords()
