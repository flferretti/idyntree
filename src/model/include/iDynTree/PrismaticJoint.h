...
public:
    ...
    IDYNTREE_DEPRECATED_WITH_MSG("Please use the setter methods to specify the parameters of the "
                                 "joint")
    PrismaticJoint(const LinkIndex link1,
                   const LinkIndex link2,
                   const Transform& link1_X_link2,
                   const Axis& _translation_axis_wrt_link1);
    ...
    /**
     * Set the prismatic axis of the joint, expressed in specified link frame, that is considered
     * the "child" frame regarding the sign of the axis.
     *
     * See getAxis method for more information.
     *
     * @warning This method should be called after a valid restTransform between link1 and link2 has
     * been set by calling the setRestTransform method.
     */
    ...
    IDYNTREE_DEPRECATED_WITH_MSG("Please use the setAxis method in which the link considered "
                                 "\"child\" is explicitly specified")
     ...
     * for more details.
     */
    virtual Axis getAxis(const LinkIndex child, const LinkIndex parent = LINK_INVALID_INDEX) const;

    // Documentation inherited
    virtual Transform getRestTransform(const LinkIndex child, const LinkIndex parent) const;
    ...
    getMotionSubspaceVector(int dof_i,
                            const LinkIndex child,
