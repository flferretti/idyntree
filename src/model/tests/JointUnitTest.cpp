...
bool getPerturbatedMatrices(const JointType& joint,
                            const VectorDynSize& jointPos,
                            const VectorDynSize& jointVel,
                            const double dt,
                            const LinkIndex linkA,
                            const LinkIndex linkB,
                            Transform& perturbatedUpper,
                            Transform& perturbatedLower)
{
    ...
    toEigen(jointPosPerturbatedUpper).segment(joint.getPosCoordsOffset(), joint.getNrOfPosCoords())
    ...
    toEigen(jointPosPerturbatedLower).segment(joint.getPosCoordsOffset(), joint.getNrOfPosCoords())
        = theta - toEigen(positionDerivative_J_velocity) * nuTheta * dt / 2;
...
template <typename JointType>
Matrix4x4 getHomTransformDerivative(const JointType& joint,
                                    const VectorDynSize& jointPos,
                                    const VectorDynSize& jointVel,
                                    const double dt,
                                    const LinkIndex linkA,
                                    const LinkIndex linkB)
{
    ...
    getPerturbatedMatrices(joint,
                           jointPos,
                           jointVel,
                           dt,
                           linkA,
                           linkB,
                           perturbatedUpperTrans,
                           perturbatedLowerTrans);
...
template <typename JointType>
Matrix6x6 getAdjTransformDerivative(const JointType& joint,
                                    const VectorDynSize& jointPos,
                                    const VectorDynSize& jointVel,
                                    const double dt,
                                    const LinkIndex linkA,
...
template <typename JointType>
Matrix6x6 getAdjTransformWrenchDerivative(const JointType& joint,
                                          const VectorDynSize& jointPos,
                                          const VectorDynSize& jointVel,
                                          const double dt,
                                          const LinkIndex linkA,
...
template <typename JointType>
void validateJointTransformDerivative(const JointType& joint,
                                      const VectorDynSize& jointPos,
                                      const VectorDynSize& jointVel,
                                      const LinkIndex linkA,
                                      const LinkIndex linkB)
{
    ...
    {
        toEigen(analyticMat) = toEigen(analyticMat)
                               + toEigen(joint.getTransformDerivative(jointPos, linkA, linkB, i)
                                             .asHomogeneousTransformDerivative())
                                     * jointPosDer(i);
    }
    ...
    Matrix4x4 homTransformDerivNum
        = getHomTransformDerivative(joint, jointPos, jointVel, numericalDerivStep, linkA, linkB);
    ...
    Matrix6x6 adjTransformDerivNum
        = getAdjTransformDerivative(joint, jointPos, jointVel, numericalDerivStep, linkA, linkB);
    ...
    Matrix6x6 adjWrenchTransformDerivNum = getAdjTransformWrenchDerivative(joint,
                                                                           jointPos,
                                                                           jointVel,
                                                                           numericalDerivStep,
                                                                           linkA,
 ...
 */
template <typename JointType>
void validateJointMotionSubspaceMatrix(const JointType& joint,
                                       ...
                                       const LinkIndex child)
{
    ...
    Eigen::Matrix<double, 6, 1> C_s_vel
        = spatialMotionEigen
          * toEigen(jointVel).segment(joint.getDOFsOffset(), joint.getNrOfDOFs());
...
template <>
VectorDynSize
setRandomJointPosition<RevoluteSO2Joint>(const RevoluteSO2Joint& joint, VectorDynSize& jointPos)
{
...
template <typename JointType> JointType getRandomJoint()
{
...
template <> SphericalJoint getRandomJoint<SphericalJoint>()
{
 ...
 */
template <typename JointType> void testJoint(bool printProgress = false)
{
    for (unsigned int i = 0; i < 10; i++)
    {
        ...
        validateJointTransformDerivative(joint,
                                         jointPos,
                                         jointVel,
        ...
        validateJointMotionSubspaceMatrix(joint,
                                          jointPos,
                                          jointVel,
                                          joint.getFirstAttachedLink(),
                                          joint.getSecondAttachedLink());
