...
{
    this->link1_X_link2
        = translation_axis_wrt_link1.getTranslationTransform(new_q) * link1_X_link2_at_rest;
...
void PrismaticJoint::resetAxisBuffers() const
{
    ...
    this->S_link2_link1
        = (link1_X_link2_at_rest.inverse() * translation_axis_wrt_link1).getTranslationTwist(1.0);
}
                                                           ...
                                                           const int posCoord_i) const
{
    ...
    TransformDerivative link1_dX_link2
        = translation_axis_wrt_link1.getTranslationTransformDerivative(dist)
          * link1_X_link2_at_rest;
...
Axis PrismaticJoint::getAxis(const LinkIndex child, const LinkIndex /*parent*/) const
{
    if (child == link1)
    {
...
void PrismaticJoint::computeChildAcc(const VectorDynSize& jntPos,
                                     const VectorDynSize& jntVel,
...
void PrismaticJoint::computeJointTorque(const VectorDynSize& jntPos,
...
bool PrismaticJoint::getPositionDerivativeVelocityJacobian(
