...
SphericalJoint::SphericalJoint()
    ...
    , m_center_wrt_link1(Position::Zero())
{
...
SphericalJoint::SphericalJoint(const SphericalJoint& other)
    ...
    , m_center_wrt_link1(other.m_center_wrt_link1)
{
...
const Transform& SphericalJoint::getTransform(const VectorDynSize& jntPos,
...
{
    // Central finite difference remains valid (quaternion renormalized), center-aware via
    // getTransform
    const double h = 1e-8;
    ...
    Matrix4x4 dH;
    toEigen(dH)
        = (toEigen(Hplus.asHomogeneousTransform()) - toEigen(Hminus.asHomogeneousTransform()))
          / (2.0 * h);
                                                            ...
                                                            const LinkIndex parent) const
{
    ...
    } else if (child == this->link1 && parent == this->link2)
    {
        ...
        if (dof_i == 0)
        {
            ...
            omega_child(0) = -mapped(0);
            omega_child(1) = -mapped(1);
            omega_child(2) = -mapped(2);
        } else if (dof_i == 1)
        {
        ...
        v_lin(0)
            = omega_child(1) * (-centerInFirstLink(2)) - omega_child(2) * (-centerInFirstLink(1));
        v_lin(1)
            = omega_child(2) * (-centerInFirstLink(0)) - omega_child(0) * (-centerInFirstLink(2));
        v_lin(2)
            = omega_child(0) * (-centerInFirstLink(1)) - omega_child(1) * (-centerInFirstLink(0));
                                        ...
                                        VectorDynSize& jntTorques) const
{
    ...
    for (int dof = 0; dof < 3; dof++)
    {
        SpatialMotionVector S
            = this->getMotionSubspaceVector(dof, linkOnWhichWrenchIsApplied, linkThatAppliesWrench);
...
bool SphericalJoint::getPositionDerivativeVelocityJacobian(
    const iDynTree::Span<const double> jntPos, iDynTree::MatrixView<double>& jac) const
{
    // Expect 4x3 view
