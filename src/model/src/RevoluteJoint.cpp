...
RevoluteJoint::RevoluteJoint()
    : link1(LINK_INVALID_INDEX)
    , link2(LINK_INVALID_INDEX)
...
{
    this->link1_X_link2
        = rotation_axis_wrt_link1.getRotationTransform(new_q) * link1_X_link2_at_rest;
...
void RevoluteJoint::resetAxisBuffers() const
{
    ...
    this->S_link2_link1
        = (link1_X_link2_at_rest.inverse() * rotation_axis_wrt_link1).getRotationTwist(1.0);
}
                                                          ...
                                                          const int posCoord_i) const
{
    ...
    TransformDerivative link1_dX_link2
        = rotation_axis_wrt_link1.getRotationTransformDerivative(ang) * link1_X_link2_at_rest;
    ...
    } else
    {
        ...
        TransformDerivative linkA_dX_linkB
            = link1_dX_link2.derivativeOfInverse(this->link1_X_link2);
...
Axis RevoluteJoint::getAxis(const LinkIndex child, const LinkIndex /*parent*/) const
{
    if (child == link1)
    {
...
void RevoluteJoint::computeChildAcc(const VectorDynSize& jntPos,
                                    const VectorDynSize& jntVel,
...
void RevoluteJoint::computeJointTorque(const VectorDynSize& jntPos,
                                       const Wrench& internalWrench,
                                       LinkIndex linkThatAppliesWrench,
                                       LinkIndex linkOnWhichWrenchIsApplied,
...
bool RevoluteJoint::getPositionDerivativeVelocityJacobian(
    const iDynTree::Span<const double> jntPos,
...
bool RevoluteJoint::hasEffortLimits() const
{
    return m_hasEffortLimits;
}

bool RevoluteJoint::enableEffortLimits(const bool enable)
{
    m_hasEffortLimits = enable;
    return true;
}

double RevoluteJoint::getEffortLimit(const size_t /*_index*/) const
{
    return m_effortLimit;
}

bool RevoluteJoint::setEffortLimit(const size_t /*_index*/, double effortLimit)
{
...
bool RevoluteJoint::hasVelocityLimits() const
{
    return m_hasVelocityLimits;
}

bool RevoluteJoint::enableVelocityLimits(const bool enable)
{
    m_hasVelocityLimits = enable;
    return true;
}

double RevoluteJoint::getVelocityLimit(const size_t /*_index*/) const
{
    return m_velocityLimit;
}

bool RevoluteJoint::setVelocityLimit(const size_t /*_index*/, double velocityLimit)
{
