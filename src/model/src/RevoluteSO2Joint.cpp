...
void RevoluteSO2Joint::resetBuffers(const double new_q_real, const double new_q_imag) const
{
    ...
    this->link1_X_link2
        = rotation_axis_wrt_link1.getRotationTransform(angle) * link1_X_link2_at_rest;
                                                             ...
                                                             const int posCoord_i) const
{
    ...
    TransformDerivative transDerivativeAngle
        = rotation_axis_wrt_link1.getRotationTransformDerivative(angle);
    Matrix4x4 link1_dX_link2_hom;
    TransformDerivative link1_dX_link2;
    toEigen(link1_dX_link2_hom)
        = toEigen((transDerivativeAngle * link1_X_link2_at_rest).asHomogeneousTransformDerivative())
          * dangle_dcoord;
...
Axis RevoluteSO2Joint::getAxis(const LinkIndex child, const LinkIndex /*parent*/) const
{
    if (child == link1)
    {
...
void RevoluteSO2Joint::computeJointTorque(const VectorDynSize& jntPos,
                                          ...
                                          VectorDynSize& jntTorques) const
{
    SpatialMotionVector S
        = getMotionSubspaceVector(0, linkOnWhichWrenchIsApplied, linkThatAppliesWrench);
...
bool RevoluteSO2Joint::getPositionDerivativeVelocityJacobian(
